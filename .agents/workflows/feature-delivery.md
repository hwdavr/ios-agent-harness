...
Gate: requirements clear, impacted files identified, API classified, UIState/Navigation designed, and all ten rule decisions are explicit. Run `bash harness/scripts/check-stage-artifacts.sh feature-delivery requirement-analysis` — must exit 0.
...
Output: `docs/current/implementation_plan_v<N>.md` created; `docs/current/test_plan_v<N>.md` created; both link the same `spec_v<N>.md#rule-applicability` record and map each `Required` rule to implementation or testing evidence; `docs/current/summary_v<N>.md` updated.
