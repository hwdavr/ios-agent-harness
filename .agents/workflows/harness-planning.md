---
description: Plan a complex iOS feature into approved vertical slices and sprint-contract artifacts.
---
...
- For UI work, read `docs/product/design_system.md` before design decisions, link it from `$FEATURE_DIR/design.md`, and record every explicit user-approved exception. Create `$FEATURE_DIR/visual_evidence/visual-target.json` as the canonical target manifest before generating mockups; it owns appearance, concrete device, logical size, locale, and named content states. Generate each mockup prompt with `bash harness/scripts/visual-target-prompt.sh --target "$FEATURE_DIR/visual_evidence/visual-target.json" --state <content_state_id>` so the mockup generator and later simulator capture consume identical target values. Generated mockups must use the manifest's exact target plus the design system's applicable tokens and component patterns.
- When an existing screen is modified, run its related, deterministic instrumented UI visual-flow test on an iOS Simulator before finalizing the design. Pull the source-fed in-test capture into `$FEATURE_DIR/design/` unchanged, then describe only the feature delta against that baseline. Record every updated surface, test file, test method, test-produced PNG name, pulled baseline path, simulator command, and passing result in `design.md`'s **Existing Surface Baseline** table. AI-generated imagery may illustrate the delta, but can never replace this baseline. If no existing test produces an in-test capture, stop planning and repair the evidence workflow before approving the design; do not use a post-test CLI screencap or a generic mockup as a substitute.
...
Gate: Update the tracker status to `Awaiting specification approval`, then run `bash harness/scripts/check-stage-artifacts.sh harness-planning feature-specification "$FEATURE_DIR"` — it validates stage artifacts, lifecycle state, existing-surface baseline evidence, the complete Rule Applicability contract, and the keyboard-visible mockup contract and must exit 0. For UI work, also verify `design.md` cites `docs/product/design_system.md`. **STOP — present specification, design document, source-fed baseline captures, and visual mockup images to user for review. Do not proceed until user explicitly approves.**
