...
| TC-US-1-VIS | AC-US-1-01 | Visual verification *(only for the final user-reachable slice with `requires_visual_verification == true`)* | Dedicated `*VisualFlowTest.swift` in-test screenshot capture | Given the production SwiftUI View is rendered in the target visual state inside a `XCUIApplication()` test, when `instrumented test.waitForIdle()` completes and `XCUIScreen.main.screenshot()` captures the active window | The in-test capture produces a non-empty PNG on device at `/var/tmp/<screen_id>_<state>.png`, pulled to `$FEATURE_DIR/visual_evidence/<screen_id>_<state>.png` via `xcrun simctl pull` for visual review against `$FEATURE_DIR/design.md` | `env xcodebuild xcodebuild test (UI Tests) -only-testing:NotesTakingAppiOSTests/<package>.<Feature>VisualFlowTest#capture<State> && xcrun simctl spawn booted cp /var/tmp/<screen_id>_<state>.png "$FEATURE_DIR/visual_evidence/<screen_id>_<state>.png" && test -s "$FEATURE_DIR/visual_evidence/<screen_id>_<state>.png"` |
